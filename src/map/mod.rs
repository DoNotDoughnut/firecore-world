use firecore_game::util::{Entity, Completable, Direction, Destination, text::{Message, TextColor}};
        ...
        if self.npc_manager.timer.is_finished() {
            ...
            if let Some(saves) = get::<PlayerSaves>() {
                ...
                for (index, npc) in self.npc_manager.npcs.iter_mut() {
                    if npc.is_alive() && !npc.should_move_to_destination() && self.npc_manager.active.map(|id| id.ne(index)).unwrap_or(true) {
